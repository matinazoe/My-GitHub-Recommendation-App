...
from recommendations.models import Project, Watchers, Review

class Recommender(object):
	allwatchers = Watchers.objects.all()
	
	def get_ids_per_user(self, user_id):
		w_user=Watchers.objects.filter(user_id=user_id)
		reviews=Review.objects.filter(user_name=user_id)
		ids_per_user = []
		for r_id in [w.repo_id for w in w_user]:
			ids_per_user.append(r_id)
		for repo_id in [r.repo for r in reviews]:
			ids_per_ids_per_user.append(repo_id)			
		return ids_per_user		
